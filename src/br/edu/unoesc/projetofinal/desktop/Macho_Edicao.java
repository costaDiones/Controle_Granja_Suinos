...
public class Macho_Edicao extends JFrame {
	...
	public void setValor(Integer posicao) {
		...
		if (machoSemen.get(posicao) instanceof Macho) {
			jrbSemen.setEnabled(false);
			Macho animal = (Macho) machoSemen.get(posicao);
			jtfMossa.setText(animal.getMossa().toString());
			jtfBrinco.setText((animal.getBrinco().toString()));
			jcbNotas.setSelectedItem(animal.getNota().getNumero().toString());
			jtfDataEntrada.setText(animal.getDataEntrada().toString());
			jtfDataNascimento.setText(animal.getDataNascimento().toString());
			jtfPesoEntrada.setText(animal.getPeso().toString());
			jtfValordeCompra.setText(animal.getValor().toString());
			jcbRaca.setSelectedItem(animal.getRaca().getNome());
			jcbFornecedor.setSelectedItem(animal.getFornecedor().getNome());
			jtfObservacao.setText(animal.getObservacao());
			jtfTetasDireitas.setText(animal.getTetasDireitas().toString());
			jtfTetasEsquerdas.setText(animal.getTetasEsquerdas().toString());
			if (animal.getTipoUtilizacao().equals("Monta Natural")) {
				jrbMontaNarutal.setSelected(true);
			} else {
				jrbInseminacao.setSelected(true);
			}
		}
		if (machoSemen.get(posicao) instanceof Semen) {
			jrbMacho.setEnabled(false);
			jrbSemen.setSelected(true);
			...
			semen = (Semen) machoSemen.get(posicao);
			jtfBrinco.setText(semen.getBrinco().toString());
			jcbNotas.setSelectedItem(semen.getNota().getNumero().toString());
			jtfDataEntrada.setText(semen.getDataEntrada().toString());
			jtfDataNascimento.setText(semen.getDataNascimento().toString());
			jcbRaca.setSelectedItem(semen.getRaca().getNome());
			jcbFornecedor.setSelectedItem(semen.getFornecedor().getNome());
		}

	}

	public Macho_Edicao(final DefaultTableModel dtmDados) {
		setLayout(null);
					...
					if (jtfMossa.getText().isEmpty() || jtfBrinco.getText().isEmpty()
						...
						for (Macho macho : machoDao.listarTodos()) {
							if (macho.getBrinco().equals(Long.valueOf(jtfBrinco.getText()))) {
								aux = 1;
								if (macho.getBrinco().equals(
										machoDao.listarTodos().get(Integer.valueOf(jtfGuardaValor.getText()))
												.getBrinco())) {
									aux = 0;
								}
								break;
						...
						if (aux == 0) {
							...
							if (nota.getValor() >= Double.valueOf(jtfValordeCompra.getText())) {
								...
								JOptionPane.showMessageDialog(null, "Macho alterado com sucesso");
								dispose();
							} else {
